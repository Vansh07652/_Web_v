---
title: "Fundamental Principles of Medication Administration"
subject: "Nursing"
...
> **Chapter:** [Chapter 11: Medication Administration](../README.md)  
> **Topic:** 2  
> **Source structure:** OpenStax public textbook outline

## Overview

A medication order tells the nurse *what* to give. The fundamental principles of medication administration are the safety framework that determines *how* it gets given correctly. They include the "rights" of medication administration, the repeated checks that verify each dose, patient identification, allergy verification, medication reconciliation, awareness of high-alert medications, protection from interruptions, respect for scope of practice, and support for the patient's right to know and to refuse.
...
The nurse confirms identity using **two identifiers**, such as the person's stated name and date of birth, compared against the ID band and the MAR. The room number is never acceptable — rooms and beds change. When the patient is able, the nurse asks them to state their name; when they cannot communicate, the nurse verifies the ID band per facility policy.

### Allergy verification

Before administering any medication, the nurse reviews the allergy record — including what the person reports the reaction to have been (rash, swelling, difficulty breathing, and so on). An allergy band or electronic flag is maintained per policy, and allergies are re-asked at every encounter, because people forget or newly develop them. Record exactly what the patient reports: an *allergy* (immune-mediated) differs from a *sensitivity* or *intolerance* (such as stomach upset), and the distinction changes how the information is used.
...
|---|---|---|
| Rights of medication administration | The standard checklist (patient, drug, dose, route, time, and more) checked before every dose | Turns "be careful" into a specific, repeatable safety check |
| Three checks | Verifying the medication against the order at retrieval, preparation, and the bedside | Repeated verification catches errors at multiple points |
| Two identifiers | Confirming identity with two separate pieces of information, such as name and date of birth | The room number is never acceptable; identity errors are deadly |
| Allergy record | The documented list of reported allergies and reactions | Must be checked before every dose and kept current |
| Medication reconciliation | Comparing all medications a person takes at every care transition | Prevents omissions, duplications, and interaction errors |
| High-alert medication | A medication category where an error is likely to cause serious harm | Triggers extra safeguards such as independent double checks |
| LASA drugs | Look-alike/sound-alike drug names that are easy to mix up | Requires extra label and verification care |
| Scope of practice | The legally and institutionally defined limits of what a nurse may do | Practicing beyond it endangers patients and the license |
| Right to refuse | The patient's right to decline a medication | Must be respected, documented, and reported |
...
|---|---|---|
| The five rights | The three checks | Rights are the checklist of what must be correct; checks are the repeated label-vs-order verifications that apply it |
| Allergy | Adverse reaction or side effect | An allergy is immune-mediated and potentially severe; side effects are expected effects; the recorded terms change how the drug is handled |
| Room number | Patient identifier | The room number identifies a bed, not a person — anyone can be in it |
| Documenting the dose | Giving the dose | Charting happens after administration; documenting first is a documentation error |
| RN medication authority | LPN/LVN or UAP authority | Routes and responsibilities vary by state law and facility policy — know your own scope |
| High-alert medication | Controlled substance | Different categories: high-alert refers to harm risk if error; controlled substance refers to regulatory scheduling — a drug can be both or either |

## Quick Review

1. Name the five classic rights of medication administration and give one example of an expanded right.
2. When do the three checks occur, and what is compared at each check?
...
1. Right patient, right drug, right dose, right route, right time. Expanded rights commonly include right reason/indication, right documentation, right response, right to refuse, and right education.
2. The checks occur when retrieving the medication from storage, while preparing it, and at the bedside before administration. Each check compares the medication label against the order or MAR (drug, dose, route, patient).
3. Rooms and beds change; a room number identifies a location, not a person. Identity must be verified with two person-specific identifiers such as name and date of birth.
4. Record exactly what the patient reports, including the nature of the reaction, and distinguish allergy from sensitivity/intolerance. Before any administration, review the allergy record and verify no ordered medication is on it.
5. Respect the refusal, explore the reason with the patient, document the refusal and the reason, and notify the prescriber so the treatment plan can be addressed.

</details>

## Related Topics

- Previous: [Medication Orders](01-medication-orders.md)
- Next: [The Medication Administration Process](03-the-medication-administration-process.md)
- Related: [Medication Administration chapter overview](../README.md)
