...
- **Exams:** Orbital speed, period-versus-altitude, geostationary orbits, and escape velocity are classic test questions.
...
The period follows from the speed: **T = 2π√(r³/GM)** — Kepler’s third law in disguise. Higher orbit means *longer* period and *slower* speed: the ISS (~400 km) laps Earth in ~90 minutes, the Moon (≈ 384,000 km) takes 27.3 days, and a satellite at 35,786 km takes exactly one sidereal day. Counterintuitive result: reaching a higher orbit requires *adding* energy, yet you end up moving *slower* — the energy goes into lifting the orbit, not speeding it up.
...
|---|---|---|
| Circular orbital velocity | The sideways speed needed to stay in a circular orbit, v = √(GM/r). | The most useful number in spaceflight; fixes launch requirements. |
| Period | The time for one complete orbit. | Links altitude to time (T = 2π√(r³/GM)); GPS, weather, and comms satellites are chosen by period. |
| Geosynchronous orbit | Any orbit with a period of one sidereal day (23 h 56 m). | Returns the satellite to the same sky position daily; the general class of 24-hour orbits. |
| Geostationary orbit | A geosynchronous orbit that is equatorial and prograde, at ≈ 35,786 km. | The satellite appears fixed over one longitude — the workhorse of weather and communications. |
| Escape velocity | The minimum speed to leave a body forever, √(2GM/r). | Determines what it takes to reach the Moon, planets, or interstellar space. |
| Delta-v | The total change in velocity a spacecraft can produce. | The currency of mission design: every orbit change has a delta-v price tag. |
| Hohmann transfer | A fuel-efficient two-burn ellipse connecting two circular orbits. | The standard route for Earth-to-geostationary and Earth-to-Mars transfers. |
| Perigee / apogee | The closest / farthest points of an Earth orbit. | The burn locations for orbit changes; drag acts at perigee. |
...
|---|---|---|
| Zero gravity in orbit | Free fall | Astronauts float because they and the station fall together; gravity there is ~90% of the surface value. |
| Satellites stay up because gravity is absent | Gravity holds them in orbit | Gravity is the *only* force keeping a satellite up — without it, the satellite would fly off in a straight line. |
| Higher orbit → faster satellite | Higher orbit → slower satellite | v = √(GM/r): larger r means smaller v. The ISS (≈ 7.8 km/s) is much faster than a geostationary satellite (≈ 3.07 km/s). |
| Geosynchronous = geostationary | Geostationary is a special case | Geosynchronous = 24-hour period at any tilt (figure-eight ground track); geostationary adds equatorial, prograde → fixed spot. |
| Escape velocity means no more gravity | Gravity still acts | Escape means you never fall back; gravity weakens with distance but never switches off. |
| Satellites need constant fuel to stay in orbit | Only for corrections | In a vacuum, no thrust is needed to keep orbiting; fuel is used for station-keeping and fighting drag at low altitude. |
...
5. Describe the two burns of a Hohmann transfer from low Earth orbit to geostationary orbit, and explain why the final orbit is slower despite adding energy.

<details>
<summary>Show answers</summary>

1. v = √(GM/r), where G is the gravitational constant, M the central body’s mass, and r the orbital radius. The satellite’s mass cancels when you equate gravity (GmM/r²) with centripetal force (mv²/r), so all satellites at the same altitude move at the same speed.
2. Fastest to slowest: ISS (≈ 7.8 km/s) > 1,000 km satellite > geostationary (≈ 3.07 km/s) > Moon. Pattern: orbital speed decreases as altitude increases.
3. Geostationary requires (1) a period of one sidereal day (23 h 56 m) and (2) an equatorial, prograde orbit. Then the satellite stays fixed over one longitude, so ground antennas never have to track it — ideal for continuous weather imaging and communications.
4. v_esc = √2 × v_circular ≈ 1.414 × 7.8 ≈ 11 km/s. Escape from the Moon is easier because the Moon is far less massive and smaller, so both M and r in √(2GM/r) work in your favor (≈ 2.4 km/s, commonly taught).
5. Burn 1 at the low orbit raises the transfer ellipse’s apogee to 35,786 km; the spacecraft coasts up, slowing as it climbs. Burn 2 at apogee raises speed enough to circularize at geostationary altitude. Both burns add kinetic energy, but the final circular speed is lower because much of the added energy was converted into gravitational potential energy by the climb — higher orbits are slower.

</details>

## Related Topics

- Previous: [Orbits in the Solar System](04-orbits-in-the-solar-system.md)
- Next: [Gravity with More Than Two Bodies](06-gravity-with-more-than-two-bodies.md)
- Related: [Newton’s Universal Law of Gravitation](03-newton-s-universal-law-of-gravitation.md) · [Orbits and Gravity chapter overview](../README.md)
