...
|---|---|---|
| Limiting reactant | The reactant that is used up first | Determines the maximum possible product |
| Excess reactant | A reactant left over when the reaction stops | Its leftover amount can be calculated and sometimes recycled |
| Theoretical yield | Maximum product mass possible from the limiting reactant | The benchmark for efficiency |
| Actual yield | Product mass actually collected | What you really got on the bench |
| Percent yield | Actual ÷ theoretical × 100% | The standard efficiency measure for a reaction |
| Side reaction | An unintended competing reaction consuming reactant | One reason actual yield falls below theoretical |
| Completion | The reaction proceeding until the limiting reactant is gone | Real reactions may stop short (equilibrium) |

## Worked Examples

### Example 1: Finding the limiting reactant and theoretical yield

Hydrogen and oxygen react to form water: \(\mathrm{2\,H_2(g) + O_2(g) \rightarrow 2\,H_2O(g)}\). Suppose 10.0 g of H₂ reacts with 80.0 g of O₂. Which reactant is limiting, and what is the theoretical yield of water? (Molar masses: H₂ = 2.016 g/mol, O₂ = 32.00 g/mol, H₂O = 18.02 g/mol.)

**Step 1 — Convert both reactants to moles:**

\[
10.0\ \text{g H}_2 \times \frac{1\ \text{mol H}_2}{2.016\ \text{g H}_2} = 4.96\ \text{mol H}_2
\]

\[
80.0\ \text{g O}_2 \times \frac{1\ \text{mol O}_2}{32.00\ \text{g O}_2} = 2.50\ \text{mol O}_2
\]

**Step 2 — Compute product from each reactant (theoretical H₂O):**

\[
4.96\ \text{mol H}_2 \times \frac{2\ \text{mol H}_2\text{O}}{2\ \text{mol H}_2} = 4.96\ \text{mol H}_2\text{O}
\]

\[
2.50\ \text{mol O}_2 \times \frac{2\ \text{mol H}_2\text{O}}{1\ \text{mol O}_2} = 5.00\ \text{mol H}_2\text{O}
\]
...
\[
4.96\ \text{mol H}_2\text{O} \times \frac{18.02\ \text{g H}_2\text{O}}{1\ \text{mol H}_2\text{O}} = 89.4\ \text{g H}_2\text{O}
\]
...
\[
\text{percent yield} = \frac{72.0\ \text{g}}{89.4\ \text{g}} \times 100\% = 80.5\%
\]
...
1. Define limiting reactant, theoretical yield, actual yield, and percent yield in one sentence each.
2. In \(\mathrm{N_2(g) + 3\,H_2(g) \rightarrow 2\,NH_3(g)}\), 1.0 mol N₂ and 4.0 mol H₂ react. Which is limiting, and how much NH₃ (in moles) can form?
3. A reaction's theoretical yield is 25.0 g and the actual yield is 20.0 g. What is the percent yield?
4. Why can a percent yield exceed 100%, and what does it usually indicate?
5. How much of the excess reactant remains in question 2?
6. Why are actual yields in real reactions almost always below theoretical?

<details>
<summary>Show answers</summary>

1. Limiting reactant: the reactant consumed first, which sets the maximum product. Theoretical yield: the maximum product mass the limiting reactant allows. Actual yield: the product mass actually collected. Percent yield: actual ÷ theoretical × 100%.
2. N₂ can make 2.0 mol NH₃; H₂ (4.0 mol ÷ 3 mol H₂ per 1 mol N₂-equivalent) can make \(4.0 \times (2/3) = 2.67\) mol NH₃. N₂ makes less, so N₂ is limiting and 2.0 mol NH₃ can form.
3. Percent yield = (20.0 g ÷ 25.0 g) × 100% = 80.0%.
4. It can only exceed 100% if the measured "product" contains extra material — water, solvent, or unreacted starting material. It signals impure product, not a reaction that beat conservation of mass.
5. H₂ consumed = 1.0 mol N₂ × (3 mol H₂ / 1 mol N₂) = 3.0 mol; remaining H₂ = 4.0 − 3.0 = 1.0 mol.
6. Reactions may not go to completion (equilibrium), side reactions consume some reactant, and product is lost during isolation and transfer — all normal, physical realities of chemistry.
