---
title: "How Do We Use Executive Functions to Make Decisions and Achieve Goals?"
subject: "Science"
...
> **Chapter:** [Chapter 19: Attention and Executive Function](../README.md)  
> **Topic:** 6  
> **Source structure:** OpenStax public textbook outline

## Overview

Executive functions are the brain's "management team": the higher-order control processes that let you plan, stay on task, ignore temptations, switch between tasks, and choose among options. They are the reason attention is useful at all — attention selects information, but executive function decides *what to do with it*. The classic framework (Miyake and colleagues) identifies three core components: **working-memory updating** (holding and revising information in mind), **inhibition** (suppressing dominant but inappropriate responses), and **cognitive flexibility/shifting** (changing your mind-set when rules change). These are supported by the **prefrontal cortex** and its loops with the basal ganglia and thalamus: the dorsolateral prefrontal cortex is central to working memory and rule use, the anterior cingulate cortex monitors conflict, and the orbitofrontal/ventromedial prefrontal cortex represents value during decision-making. This topic explains how these pieces work together to produce goal-directed behavior, how they are measured, and what happens when they fail.

## Why This Matters

Executive functions predict real-world outcomes: they are among the best cognitive predictors of academic achievement, job performance, health behavior, and even relationship stability. They let a student ignore a buzzing phone while studying, a driver hold a route plan while navigating, and a clinician weigh contradictory evidence before acting. When executive function fails — after frontal damage, in ADHD, in dementia, or in normal fatigue — goal-directed behavior collapses into impulsivity, perseveration, or apathy, which is why executive assessment (Stroop, Wisconsin Card Sorting, go/no-go, planning tasks) is central to neuropsychological testing. The neuroscience also demystifies everyday experience: "multitasking" costs, difficulty breaking a habit, and the effort of self-control all have neural accounts. This topic closes the chapter by showing how attention, working memory, and decision-making converge in the prefrontal cortex.
...
- **Anterior cingulate cortex (ACC)**: conflict monitoring and error detection. The ACC registers when two responses compete (the Stroop conflict) and when you make an error (the error-related negativity, an EEG component), signaling other regions to increase control.
...
- **Value-based choice**: options are represented by their expected value (probability × payoff), and the brain's choice system compares these representations. Dopamine neurons are widely taught as encoding **reward prediction error** — the difference between the reward you got and the reward you expected — which drives learning and updating (a model, not a literal readout).
...
|---|---|---|
| Executive function | Higher-order control processes (updating, inhibition, shifting) that direct goal-oriented behavior | The management layer that makes attention, memory, and action serve goals |
| Updating | Monitoring and revising the contents of working memory | Keeps goal-relevant information current as the situation changes |
| Inhibition | Suppressing automatic or dominant responses | The basis of self-control; measured by Stroop and go/no-go |
| Shifting | Switching between tasks, rules, or mental sets | Flexibility; measured by task-switching and the Wisconsin Card Sorting Test |
| Working memory | Active maintenance and manipulation of information (~4 ± 1 chunks, commonly taught) | The workspace where goals, attention, and memory meet |
| Dorsolateral PFC (dlPFC) | Frontal region supporting working memory, rules, and planning | The "workhorse" of cognitive control |
| Anterior cingulate cortex (ACC) | Region detecting conflict and errors | Signals when more control is needed; source of the error-related negativity |
| Orbitofrontal/ventromedial PFC (OFC/vmPFC) | Frontal regions representing value and guiding decisions | Damage → poor choices, reversal-learning perseveration |
| Reward prediction error | Difference between expected and received reward, encoded by dopamine neurons (commonly taught) | The learning signal that updates choices |
| Perseveration | Repeating the same response despite changed rules or outcomes | Classic sign of frontal/executive dysfunction |
| Switch cost | Slower/less accurate performance after changing tasks | Why "multitasking" degrades performance |
...
1. Name the three core executive components of the Miyake framework and one task that measures each.
2. Which prefrontal regions are associated with (a) working memory/planning, (b) conflict monitoring, and (c) value-based decision-making?
3. What is a switch cost, and what does it imply about "multitasking"?
4. How does reward prediction error relate to decision-making and learning?
5. Give two behavioral signs of prefrontal/executive dysfunction and explain each.

<details>
<summary>Show answers</summary>

1. Updating (working memory) — N-back or keep-track tasks; inhibition — Stroop or go/no-go; shifting — task-switching paradigms or the Wisconsin Card Sorting Test.
2. (a) Dorsolateral prefrontal cortex; (b) anterior cingulate cortex; (c) orbitofrontal/ventromedial prefrontal cortex.
3. A switch cost is the slower, less accurate performance that follows changing tasks, because the executive system must reconfigure the task set. It implies that "multitasking" is really rapid task-switching, and that dividing attention between demanding tasks degrades performance — important for driving and safety-critical clinical work.
4. Dopamine neurons are commonly taught to encode reward prediction error — the difference between expected and received reward — which updates the value representations that guide future choices, allowing you to learn which options pay off.
5. Perseveration: repeating a response after the rules or contingencies have changed (e.g., failing reversal learning). Disinhibition: acting without regard to goals (failing to suppress dominant responses). Apathy: failure to initiate goal-directed action. All reflect breakdowns of updating, inhibition, and/or shifting after frontal damage.

</details>

## Related Topics

- Previous: [How Do Clinical Disorders Affect Attentional Function?](05-how-do-clinical-disorders-affect-attentional-function.md)
- Next: End of chapter
- Related: [Attention and Executive Function chapter overview](../README.md)
...
- Content type: Original educational study guide based on OpenStax outline structure. The Miyake three-component model, PFC sector functions, working-memory capacity (~4 ± 1 chunks), reward prediction error, and the Iowa Gambling Task/somatic marker hypothesis are standard textbook material; treat the somatic marker hypothesis and reward-prediction-error account as influential models rather than settled facts, and verify numeric values against current texts.
