...
|---|---|---|
| Sarcomere | Repeating contractile unit between Z-lines | The functional unit where filament sliding produces shortening |
| Myosin | Thick filament with cross-bridge heads | The motor protein that pulls actin during contraction |
| Actin | Thin filament with binding sites for myosin | The track that myosin heads pull on |
| Troponin / tropomyosin | Regulatory proteins on the thin filament | Calcium binds troponin, which moves tropomyosin to expose actin binding sites |
| Sarcoplasmic reticulum (SR) | Calcium storehouse inside the muscle fiber | Releases Ca²⁺ to trigger contraction; pumps it back for relaxation |
| T-tubules | Membrane invaginations that carry the signal deep into the fiber | Deliver the action potential to the SR so all sarcomeres activate together |
| Neuromuscular junction | Synapse between a motor neuron and a muscle fiber | Where ACh triggers the muscle action potential |
| Motor unit | One motor neuron plus all the fibers it innervates | The basic unit of force; recruitment of more units = more force |
| Twitch / tetanus | Single brief contraction / sustained contraction from rapid stimulation | Explains how force is graded and how steady contractions are maintained |
| Isotonic / isometric | Length change with tension / tension without length change | Two fundamental ways muscles generate force |
| Creatine phosphate | High-energy molecule that quickly regenerates ATP | Powers the first seconds of intense exercise |
| Rigor mortis | Post-mortem stiffening from cross-bridges stuck without ATP | Forensic application of the cross-bridge cycle |
| Intercalated discs | Specialized junctions between cardiac muscle cells | Couple heart cells electrically so the heart beats as one unit |

## Example: The Biceps Curl, from Brain to Burn
...
|---|---|---|
| Actin | Myosin | Actin is the thin filament being pulled; myosin is the thick filament whose heads do the pulling. |
| Troponin | Tropomyosin | Calcium binds *troponin*, which then shifts *tropomyosin* off the actin binding sites. Troponin = sensor; tropomyosin = gate. |
| Contraction = shortening | All contractions | Isotonic contractions shorten; isometric contractions produce tension with no length change; eccentric contractions lengthen under load. |
| Twitch | Tetanus | Twitch is one brief contraction; tetanus is a sustained contraction from rapid successive stimuli (the normal steady state of voluntary muscle). |
| ATP powers the power stroke only | ATP's multiple roles | ATP also detaches myosin from actin and powers the calcium pump; without ATP, cross-bridges stay stuck (rigor mortis). |
...
| Skeletal muscle | Cardiac muscle | Skeletal is voluntary and multinucleated; cardiac is involuntary, striated, and electrically coupled by intercalated discs. |
| Flexion | Contraction | Flexion is a joint movement (decreasing the angle); contraction is the cellular event in the muscle that produces it. |
...
1. During contraction, myosin heads pull thin actin filaments past thick myosin filaments, so the sarcomere shortens (Z-lines move closer together; I bands and H zones narrow). The filaments themselves do not change length — they slide.
2. Attach (myosin head binds actin), power stroke (head pivots, pulling actin), detach (ATP binds myosin, releasing it), re-cock (ATP hydrolysis re-energizes the head). Without ATP, myosin cannot detach — cross-bridges remain locked, producing rigor mortis after death.
3. ACh released at the neuromuscular junction → muscle action potential along the sarcolemma → propagation down T-tubules → Ca²⁺ release from the sarcoplasmic reticulum → Ca²⁺ binds troponin → tropomyosin shifts off actin binding sites → cross-bridges form and the sarcomere shortens.
4. Creatine phosphate (regenerates ATP instantly — powers the first seconds), fast glycolysis (quick ATP without oxygen, producing lactic acid — high-intensity minutes), and aerobic respiration (slow but abundant ATP with oxygen — endurance). Creatine phosphate is the immediate source.
5. Isotonic: the muscle changes length while tension is roughly constant (lifting the dumbbell — concentric). Isometric: tension develops with no length change (holding the dumbbell steady). Eccentric: the loaded muscle lengthens (lowering the dumbbell slowly), the phase most responsible for delayed muscle soreness.
6. A motor unit is one motor neuron plus all the muscle fibers it innervates. Force increases by recruiting more motor units and by increasing firing rate (summation toward tetanus), so the nervous system grades force by turning on more units rather than changing how hard individual fibers pull.

</details>

## Related Topics

- Previous: [Joints and Skeletal Movement](03-joints-and-skeletal-movement.md)
- Next: End of chapter
- Related: [The Musculoskeletal System chapter overview](../README.md)
...
- Content type: Original educational study guide based on OpenStax outline structure. Physiological descriptions follow the standard sliding-filament model as commonly taught; verify specific quantitative claims (ATP yields, time courses) against current texts.
