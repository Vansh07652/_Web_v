...
- **Therapy logic:** Many cancer drugs target the regulators in this chapter — kinase inhibitors, proteasome inhibitors, and drugs that restore apoptosis (commonly taught examples).
...
- **AP® exam logic:** Oncogene vs. tumor suppressor, gain- vs. loss-of-function, and the multistep model of cancer are high-frequency free-response concepts.
...
Cell division is governed by the **cell cycle**, whose transitions are driven by **cyclins** and **cyclin-dependent kinases (CDKs)**. Checkpoints pause the cycle when problems are detected: the **G1/S checkpoint** checks conditions before DNA is copied, the **G2/M checkpoint** verifies replication was damage-free, and the **M checkpoint** confirms chromosomes are attached correctly. Two tumor suppressors police these gates:
...
|---|---|---|
| Proto-oncogene | Normal gene that promotes cell growth/division | The raw material; only dangerous when mutated |
| Oncogene | Mutated, permanently active growth-promoting gene | Drives uncontrolled proliferation; gain of function |
| Tumor suppressor gene | Normal gene that restrains division, promotes repair/death | Its loss removes the brakes; loss of function |
| Gain of function / loss of function | Mutation that adds activity / removes activity | Oncogenes = gain; tumor suppressors = loss — opposite logic |
| Checkpoint | Cell-cycle gate that pauses division if problems are found | The surveillance system cancer cells must defeat |
| Rb | Tumor suppressor that blocks entry into S phase | The G1→S brake; classic two-hit gene (retinoblastoma) |
| p53 | Tumor suppressor that arrests the cycle or triggers apoptosis after DNA damage | "Guardian of the genome"; mutated in a large share of human cancers |
| Apoptosis | Programmed, orderly cell death | The fail-safe that eliminates damaged cells |
| Bcl-2 family | Proteins that promote (Bax) or block (Bcl-2) apoptosis | The molecular switch cancer cells flip to avoid death |
| Angiogenesis | Growth of new blood vessels into a tumor | Supplies the tumor; a target of cancer drugs |
| Metastasis | Spread of cancer cells to distant tissues | What makes cancer lethal; requires invasion and migration |
| Two-hit hypothesis | Both copies of a tumor suppressor must be lost | Explains inherited cancer risk and why tumor suppressors behave recessively |

## Example: The Two-Hit Story of Retinoblastoma
...
1. Distinguish proto-oncogenes, oncogenes, and tumor suppressor genes in terms of normal function and mutation outcome.
2. Explain the two-hit hypothesis and why it predicts earlier, more frequent tumors in people who inherit one defective tumor-suppressor allele.
3. What are the two main decisions p53 can make after DNA damage, and what determines which one occurs?
4. List three ways a proto-oncogene can become an oncogene.
5. Why must apoptosis fail for cancer to develop, and which protein family controls the decision?
6. Why does cancer risk increase with age, according to the multistep model?

<details>
<summary>Show answers</summary>

1. Proto-oncogenes normally promote growth; mutation converts them into oncogenes that are stuck on (gain of function, dominant). Tumor suppressors normally restrain division/promote repair or death; their loss (both copies) removes the brakes (loss of function, recessive at the cellular level).
2. Both copies of a tumor suppressor must be inactivated for its protection to disappear. An inherited defective copy means every cell already has one hit, so one somatic mutation anywhere delivers the second hit — making tumors earlier and more numerous than in people who need two somatic hits in the same cell.
3. p53 can arrest the cell cycle at G1 to allow DNA repair, or trigger apoptosis if damage is too severe to repair — the severity of the damage drives the decision.
4. Point mutation (e.g., Ras locked on), gene amplification (extra copies → more protein), and chromosomal translocation (e.g., BCR-ABL fusion) — plus overexpression driven by misplaced regulatory elements.
5. Because a cell with a broken cycle can still die by apoptosis, eliminating it; cancer cells must disable that fail-safe (e.g., lose p53 or overexpress anti-apoptotic Bcl-2). The Bcl-2 family (Bax promotes, Bcl-2 blocks) controls the decision.
6. Mutations accumulate with every cell division over a lifetime; cancer requires multiple hits across several gene classes, so the probability that one cell has acquired the full set rises steadily with age.

</details>

## Related Topics

- Previous: [Eukaryotic Translational and Post-translational Gene Regulation](06-eukaryotic-translational-and-post-translational-gene-regulation.md)
- Related: [Eukaryotic Epigenetic Gene Regulation](03-eukaryotic-epigenetic-gene-regulation.md) (silenced tumor suppressors)
- Related: [Eukaryotic Transcriptional Gene Regulation](04-eukaryotic-transcriptional-gene-regulation.md) (Myc, transcription-factor oncogenes)
- Related: [Control of the Cell Cycle](../chapter-10-cell-reproduction/03-control-of-the-cell-cycle.md) (cyclins, CDKs, checkpoints)
- Related: [Cancer and the Cell Cycle](../chapter-10-cell-reproduction/04-cancer-and-the-cell-cycle.md)
- Related: [Gene Regulation chapter overview](../README.md)
...
- Content type: Original educational study guide based on OpenStax outline structure. Gene names and model systems (Ras, Rb, p53, BCR-ABL, colorectal progression, HPV E6) are standard, commonly taught reference concepts; verify specific details and any clinical claims against current texts.
