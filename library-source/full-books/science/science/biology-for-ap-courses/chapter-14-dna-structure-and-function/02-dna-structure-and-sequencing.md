...
Watson and Crick's 1953 model (built on Franklin's X-ray data and Chargaff's rules) described DNA as two **antiparallel** strands — one running 5′ → 3′, the other 3′ → 5′ — wound around a common axis. The sugar-phosphate backbones form the outside; the paired bases stack inside like the rungs of a twisted ladder. Commonly taught dimensions: the helix is about **2 nm in diameter**, bases are stacked about **0.34 nm apart**, and there are about **10 base pairs per turn**. The strands are **complementary** — the sequence of one determines the sequence of the other — which is the basis of copying and hybridization.
...
|---|---|---|
| Nucleotide | Phosphate + 5-carbon sugar + nitrogenous base | The repeating unit of DNA and RNA. |
| Purine / Pyrimidine | Two-ring bases (A, G) / one-ring bases (C, T) | Purine–pyrimidine pairing keeps the helix width constant. |
| Phosphodiester bond | Link between the 5′ phosphate and 3′ hydroxyl of adjacent nucleotides | Forms the sugar-phosphate backbone. |
| 5′ / 3′ ends | The two directional ends of a nucleotide chain | Defines DNA directionality; synthesis always runs 5′ → 3′. |
| Antiparallel | The two strands run in opposite directions | Required for base pairing; central to replication and transcription. |
| Complementary | One strand's sequence determines the other's (A–T, G–C) | Basis of replication, sequencing, and hybridization. |
| Dideoxynucleotide (ddNTP) | Nucleotide that terminates chain growth | The key reagent in Sanger sequencing. |
| Sequencing | Reading the order of bases in a DNA molecule | Underlies genomics, forensics, and clinical genetics. |
| Nucleosome | DNA wrapped around histone proteins | First level of packaging a 2 m genome into a nucleus. |

## Example: Reading a sequence with Sanger logic
...
1. Phosphate group, deoxyribose sugar, and a nitrogenous base. The bases are adenine (A), thymine (T), guanine (G), and cytosine (C).
2. A purine (two rings) pairing with a pyrimidine (one ring) keeps the sugar-phosphate backbones a constant distance apart, giving the helix its uniform ~2 nm width. Purine–purine pairs would bulge; pyrimidine–pyrimidine pairs would pinch.
3. One strand runs 5′ → 3′ while the other runs 3′ → 5′. Directionality matters because polymerases synthesize DNA only in the 5′ → 3′ direction, which shapes how replication and transcription work.
4. RNA uses ribose instead of deoxyribose, uracil instead of thymine, and is usually single-stranded rather than double-stranded.
5. A ddNTP lacks the 3′ hydroxyl, so once it is added to a growing strand no further nucleotides can attach — the chain terminates. The lengths of terminated fragments reveal the position of each base.

</details>

## Related Topics

- Previous: [Historical Basis of Modern Understanding](01-historical-basis-of-modern-understanding.md)
- Next: [Basics of DNA Replication](03-basics-of-dna-replication.md)
