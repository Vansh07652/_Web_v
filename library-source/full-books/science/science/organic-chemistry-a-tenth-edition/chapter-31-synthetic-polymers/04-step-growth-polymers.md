...
**Step-growth polymerization** builds polymers by the stepwise reaction of functional groups: any molecule bearing the right functional groups can react with any other, dimer forming trimer, trimer adding to pentamer, and so on. The classic reactions are **esterification** (a diacid + a diol → **polyester**) and **amidation** (a diacid + a diamine → **polyamide**). Nylon-6,6 — made from adipic acid and hexamethylenediamine — and PET (polyethylene terephthalate, from terephthalic acid and ethylene glycol) are the flagship products, alongside polycarbonates and polyurethanes.

The defining contrast with chain growth (Topic 1) is kinetics. In step growth, **molecular weight builds up slowly and only becomes high at very high conversion**: a chain of 100 units requires 99 successful coupling events, each depending on the same statistical coupling of functional groups. Early in the reaction the mixture is full of short oligomers; only when 99%+ of the functional groups have reacted do chains become long. The relationship between conversion \(p\) (fraction of functional groups that have reacted) and the average degree of polymerization is the **Carothers equation**:

\[
\overline{DP} = \frac{1}{1 - p}
\]

This single equation is the heart of the topic: it says that to reach \(\overline{DP} = 100\), you need \(p = 0.99\) — 99% conversion. It also explains why step-growth polymers are almost always made with *exactly* balanced stoichiometry and why industrial processes drive water (or other small molecules) out of the reaction to push conversion higher.
...
- \(p = 0.90\) → \(\overline{DP} = 10\) (a useless oligomer).
- \(p = 0.99\) → \(\overline{DP} = 100\) (a useful low polymer).
- \(p = 0.999\) → \(\overline{DP} = 1000\) (a high polymer).
...
- **Carothers equation:** \(\overline{DP} = 1/(1-p)\). \(p = 0.99\) → DP = 100; \(p = 0.999\) → DP = 1000.
...
|---|---|---|
| Step-growth polymerization | Polymerization by random coupling of functional groups on any two molecules | Mechanism of polyesters, polyamides, polycarbonates, polyurethanes |
| Condensation polymerization | Step growth that eliminates a small molecule per coupling | The byproduct must be removed to drive the reaction to high conversion |
| Carothers equation | \(\overline{DP} = 1/(1-p)\), relating conversion to chain length | Quantifies why near-quantitative conversion is required for useful polymers |
| Conversion \(p\) | Fraction of functional groups that have reacted | The master variable controlling DP in step growth |
| Polyester | Polymer with –C(=O)–O– links (diacid + diol) | PET bottles and polyester fibers |
| Polyamide | Polymer with –C(=O)–NH– links (diacid + diamine) | Nylon and Kevlar; strong via hydrogen bonding |
| Nylon-6,6 | Polyamide of adipic acid + hexamethylenediamine | Classic engineering polymer; "6,6" = six carbons per monomer |
| Thermoset | Cross-linked network polymer that cannot be remelted | Formed when monomers have >2 functional groups; not recyclable by melting |
...
\[
\overline{DP} = \frac{1}{1 - p} = \frac{1}{1 - 0.95} = \frac{1}{0.05} = 20
\]

**Step 2 — Solve for the conversion needed at DP = 200.** Rearrange:

\[
200 = \frac{1}{1 - p} \Rightarrow 1 - p = \frac{1}{200} = 0.005 \Rightarrow p = 0.995
\]

**Answer.** At 95% conversion, \(\overline{DP} = 20\) (an oligomer). A DP of 200 requires \(p = 0.995\) — 99.5% of all functional groups must react, showing why industrial step growth is pushed to near-completion.
...
1. Write the Carothers equation and compute \(\overline{DP}\) at \(p = 0.98\).
2. Why does step-growth molecular weight stay low until very high conversion?
3. What happens to nylon molecular weight if the diamine is charged 2% short, and why?
4. Name the monomers and repeat-unit linkage of PET and of nylon-6,6.
5. Why must the condensation byproduct be removed during the reaction?
6. What structural feature converts a step-growth polymer into a thermoset?

<details>
<summary>Show answers</summary>

1. \(\overline{DP} = 1/(1-p)\); at \(p = 0.98\), \(\overline{DP} = 1/0.02 = 50\).
2. Because chain length grows by statistical coupling of functional groups: reaching DP = 1000 requires 999 successive couplings, and the average length only climbs when nearly all functional groups have reacted.
3. The diamine runs out first, leaving excess diacid to cap every chain end with –COOH; the average chain length is severely limited and the polymer is too short for strong fibers.
4. PET: ethylene glycol + terephthalic acid, ester links (–C(=O)–O–). Nylon-6,6: hexamethylenediamine + adipic acid, amide links (–C(=O)–NH–).
5. Esterification and amidation are equilibria; removing the small-molecule byproduct (water) shifts the equilibrium toward polymer (Le Chatelier), raising conversion and therefore DP.
6. Monomers with more than two functional groups (e.g., a triol) allow branching that links chains into a cross-linked network, which cannot melt or dissolve.

</details>

## Related Topics

- Previous: [Copolymers](03-copolymers.md)
- Next: [Olefin Metathesis Polymerization](05-olefin-metathesis-polymerization.md)
- Related: [Chain-Growth Polymers](01-chain-growth-polymers.md) (the contrasting mechanism), [Polyamides and Polyesters: Step-Growth Polymers](../../chapter-21-carboxylic-acid-derivatives-nucleophilic-acyl-substitution-reactions/09-polyamides-and-polyesters-step-growth-polymers.md) (the acyl-substitution chemistry behind the links), [Polymer Structure and Physical Properties](07-polymer-structure-and-physical-properties.md)
...
- Content type: Original educational study guide based on OpenStax outline structure. The Carothers equation and its conversion dependence, the balanced-stoichiometry requirement, and the PET/nylon-6,6 monomer identities are standard textbook polymer chemistry (W. H. Carothers, DuPont, 1930s). The approximate non-stoichiometric DP estimate in Example 2 is a textbook limiting-reagent treatment; actual industrial processes control stoichiometry far more tightly.
- Lab safety note: diacids, diamines, and diols can be corrosive or irritating; polymerization at elevated temperature requires proper ventilation, heat control, and PPE per institutional rules (general principles only).
