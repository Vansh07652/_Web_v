...
> **Chapter:** [Chapter 27: Biomolecules: Lipids](../README.md)  
> **Topic:** 6  
> **Source structure:** OpenStax public textbook outline

## Overview

**Steroids** are lipids built on a tetracyclic skeleton called **cyclopentanoperhydrophenanthrene** — three fused six-membered rings (A, B, C) plus one five-membered ring (D). Because they are large, rigid, fused-ring molecules rather than open chains, steroids pack into membranes with defined orientation, and small changes in functional groups produce hormones with dramatically different effects.

The parent compound in animals is **cholesterol** (\\(C_{27}H_{46}O\\)): a hydroxyl at C-3, a C-5/C-6 double bond, two angular methyls (C-18, C-19), and an eight-carbon side chain at C-17. Every steroid hormone in humans is biosynthetically derived from cholesterol, so the core skeleton is shared even when the functional groups differ.
...
- A hydroxyl group at C-3 (β), making the molecule **amphipathic**: the OH is polar, while the fused-ring body and side chain are nonpolar.
...
- **Progesterone** (\\(C_{21}H_{30}O_2\\)): ketones at C-3 and C-20 — a **pregnane** skeleton; maintains pregnancy.
- **Cortisol** (\\(C_{21}H_{30}O_5\\)): a **glucocorticoid** with hydroxyls at C-11β, C-17α, C-21 and ketones at C-3, C-20; mobilizes glucose and suppresses inflammation.
- **Aldosterone** (\\(C_{21}H_{28}O_5\\)): a **mineralocorticoid** with an aldehyde at C-18; controls sodium and potassium balance.
- **Testosterone** (\\(C_{19}H_{28}O_2\\)): an **androgen** with a C-3 ketone, a C-17β-OH, and a C-4/C-5 double bond; 19 carbons.
- **Estradiol** (\\(C_{18}H_{24}O_2\\)): an **estrogen** with an *aromatic* A ring (phenol at C-3) and a C-17β-OH; 18 carbons because the C-19 methyl is lost on aromatization. Aromatic ring A is the most reliable clue that a steroid is an estrogen.
...
- Cholesterol: \\(C_{27}H_{46}O\\), OH at C-3β, C-5/C-6 double bond, angular methyls C-18/C-19, 8-carbon side chain at C-17; parent of all steroid hormones.
...
|---|---|---|
| Steroid | Lipid with the tetracyclic cyclopentanoperhydrophenanthrene skeleton | Defines an entire hormone family with shared shape and membrane behavior |
| Sterol | Steroid carrying a hydroxyl group | Cholesterol is a sterol; the OH confers amphipathicity |
| Cyclopentanoperhydrophenanthrene | Four fused saturated rings: three six-membered + one five-membered | The skeleton every steroid is built on |
| Angular methyl | Methyl on a ring junction (C-18 at C-13, C-19 at C-10) | Distinguishes estrogens (C-19 lost) |
| β / α substituent | Group projecting out of (β) or away from (α) the ring plane | Stereochemistry determines receptor binding |
| Amphipathic | Having both polar and nonpolar regions | Explains cholesterol's membrane role and bile acids' detergent action |
| Bile acid | Steroid with a C-17 carboxylate side chain that emulsifies fats | Connects steroid chemistry to digestion of dietary lipids |

## Worked Examples

### Example 1: Degree of unsaturation of cholesterol

Cholesterol has the molecular formula \\(C_{27}H_{46}O\\). Calculate its degree of unsaturation (DBE) and show that it is consistent with four rings plus one double bond.
...
\[
\text{DBE} = \frac{2(27) + 2 - 46}{2} = \frac{54 + 2 - 46}{2} = \frac{10}{2} = 5
\]
...
\[
M = 27(12.011) + 46(1.008) + 1(15.999)\ \text{g/mol}
\]

**Compute:**

\[
M = 324.30 + 46.37 + 15.999 = 386.66\ \text{g/mol} \approx 386.7\ \text{g/mol}
\]

**Molecules in 1.00 g** — set up the unit conversion so grams cancel:

\[
1.00\ \text{g} \times \frac{1\ \text{mol}}{386.66\ \text{g}} \times \frac{6.022 \times 10^{23}\ \text{molecules}}{1\ \text{mol}} = 1.56 \times 10^{21}\ \text{molecules}
\]
...
A steroid has formula \\(C_{18}H_{24}O_2\\), a phenolic hydroxyl on an aromatic A ring, and a β-hydroxyl at C-17. Is this compound an androgen, estrogen, or glucocorticoid?
...
|---|---|---|
| Steroid | Sterol | A sterol is a steroid with a hydroxyl group; all sterols are steroids, but not all steroids are sterols |
...
| Cholesterol in membranes | Cholesterol as "bad" | Cholesterol is essential for membranes and hormone synthesis; "bad cholesterol" means its LDL transport lipoprotein, not the molecule |
| β vs α substituent | Up vs down on the page | β = solid wedge (out of page); α = dashed wedge (into page); wedge direction matters for activity |
| Bile acid | Fatty acid soap | Both are amphipathic detergents, but bile acids have a rigid steroid scaffold and a C-17 carboxylate side chain, not a long flexible alkyl tail |
| Steroid hormones | Peptide hormones | Steroids are small, fat-soluble, act via intracellular receptors, altering gene expression; peptide hormones act at cell-surface receptors |
...
3. Calculate the degree of unsaturation of testosterone, \\(C_{19}H_{28}O_2\\).
...
1. A, B, C are six-membered; D is five-membered; together they provide the 17 ring carbons.
2. Estrogens: 18 carbons, aromatic A ring, OH at C-17. Androgens: 19 carbons, nonaromatic A ring with C-3 ketone, β-OH at C-17.
3. DBE = \\([2(19) + 2 - 28]/2 = 12/2 = 6\\), matching four rings plus two double-bond equivalents (the C-4/C-5 alkene and the C-3 carbonyl).
4. The C-3 OH is polar; rings and side chain are nonpolar. In the bilayer the OH faces water while flat rings pack against phospholipid tails, stiffening the membrane.
5. Amphipathic: hydroxyls + C-17 carboxylate (polar face) and steroid rings (nonpolar face) emulsify fat droplets into micelles.

</details>

## Related Topics

- Previous: [Terpenoids](05-terpenoids.md)
- Next: [Biosynthesis of Steroids](07-biosynthesis-of-steroids.md)
- Related: [Biomolecules: Lipids chapter overview](../README.md)
