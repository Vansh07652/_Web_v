...
Interpreting an IR spectrum is systematic pattern matching, not memorization of every band. The spectrum has two working regions: the **functional-group region** (4000–1400 cm⁻¹), where the diagnostic stretches of C–H, O–H, N–H, C≡N, C=O, and C=C appear, and the **fingerprint region** (below ~1400 cm⁻¹), a dense forest of bending and skeletal vibrations unique to each molecule — as individual as a human fingerprint.
...
- **Broad bands** signal hydrogen bonding: O–H stretches are wide humps (3200–3600 cm⁻¹); carboxylic acid O–H is even broader (2500–3300 cm⁻¹). N–H bands are narrower.
...
- Quantitative link: \(A = 2 - \log_{10}(\%T)\) and \(A = \varepsilon b c\) (Beer–Lambert).

## Key Terms

| Term | Plain-Language Definition | Why It Matters |
|---|---|---|
| Functional-group region | 4000–1400 cm⁻¹, where diagnostic stretches appear | Where you identify which functional groups are present |
| Fingerprint region | Below ~1400 cm⁻¹, dense unique band pattern | Used to confirm identity, not to assign groups |
| Transmittance (%T) | Percent of IR light passing through the sample | Absorption appears as downward dips from 100% T |
| Absorbance (\(A\)) | \(2 - \log_{10}(\%T)\); scales linearly with concentration | Enables quantitative IR analysis via Beer–Lambert |
| Hydrogen bonding | O–H (and N–H) groups associating via H-bonds | Broadens and shifts O–H bands — a key shape clue |
| Carbonyl window | 1630–1800 cm⁻¹, where C=O stretches appear | The anchor band for functional-group identification |
| Diagnostic band | A band whose position identifies a functional group | The core of spectrum interpretation |
| Elimination reasoning | Using absent bands to rule out functional groups | Often faster and more reliable than assigning every peak |
...
**Reasoning:** The 1715 cm⁻¹ band lands in the ketone range of the carbonyl window. No broad O–H rules out alcohol and carboxylic acid; no aldehyde C–H doublet rules out an aldehyde; no strong C–O companion pattern rules out an ester. The structure is consistent with **butanone (methyl ethyl ketone)**, \(\text{CH}_3\text{C(=O)CH}_2\text{CH}_3\).
...
A carbonyl band reads 5% T through a cell of path length \(b = 0.10\ \text{cm}\). Find the absorbance, then the concentration if the molar absorptivity is \(\varepsilon = 200\ \text{L·mol}^{-1}\text{·cm}^{-1}\).

**Step 1 — absorbance from transmittance:**

\[
A = 2 - \log_{10}(\%T) = 2 - \log_{10}(5) = 2 - 0.70 = 1.30
\]

**Step 2 — Beer–Lambert, solved for \(c\):**

\[
A = \varepsilon b c \quad \Rightarrow \quad c = \frac{A}{\varepsilon b}
\]

Substitute with units:

\[
c = \frac{1.30}{(200\ \text{L·mol}^{-1}\text{·cm}^{-1})(0.10\ \text{cm})} = 0.065\ \text{mol/L}
\]

Dimensional check: \(A\) is unitless; \(\text{L·mol}^{-1}\text{·cm}^{-1} \times \text{cm} = \text{L·mol}^{-1}\), so \(1/(\text{L·mol}^{-1}) = \text{mol/L}\). ✓ The linear \(A\)-vs-\(c\) relationship is why quantitative work uses absorbance, never raw percent transmittance.

## Common Confusions

| Do not confuse | With | Difference |
|---|---|---|
| Broad O–H band | Sharp N–H band | O–H is a wide hump (H-bonding); N–H is sharper; two N–H bands mean a primary amine |
| Alcohol O–H | Carboxylic acid O–H | Acid O–H is broader (~2500–3300 cm⁻¹) and pairs with C=O near 1710 cm⁻¹ |
| Absence of C=O | "No information" | Absence is strong evidence: no band in 1630–1800 cm⁻¹ rules out all carbonyl groups |
| Band above 3000 cm⁻¹ | Saturated C–H | Above 3000 = sp²/aromatic C–H; saturated C–H sits at 2850–2960 cm⁻¹ |
| Nitrile C≡N | Alkyne C≡C | C≡N is medium-sharp near 2240 cm⁻¹; C≡C is characteristically weak |
| The 2720/2820 cm⁻¹ doublet | C–H overtones | It is the aldehyde C–H — a reliable "aldehyde signature" |
...
1. Functional-group region (4000–1400 cm⁻¹) for identifying groups; fingerprint region (<1400 cm⁻¹) for confirming identity.
2. A carboxylic acid: the C=O near 1710 cm⁻¹ plus the exceptionally broad O–H (2500–3300 cm⁻¹) is the acid signature.
3. Terminal alkyne C–H is a sharp band near 3300 cm⁻¹ (often with a weak C≡C band ~2100–2140 cm⁻¹); aromatic C–H is weaker, near 3030–3100 cm⁻¹, with ring bands at 1450–1600 cm⁻¹.
4. \(A = 2 - \log_{10}(10) = 2 - 1 = 1.00\).
5. The C=O stretch is strong and sits in an otherwise quiet region (1630–1800 cm⁻¹); if it is absent, no carbonyl-containing group can be present — absence rules out an entire family at once.
</details>

## Related Topics

- Previous: [Infrared Spectroscopy](06-infrared-spectroscopy.md)
- Next: [Infrared Spectra of Some Common Functional Groups](08-infrared-spectra-of-some-common-functional-groups.md)
