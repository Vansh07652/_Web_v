...
Carbonyl compounds are much stronger acids than alkanes because of their **α-hydrogens**: deprotonation gives an **enolate ion**, a resonance-stabilized species with negative charge shared between the α carbon and the carbonyl oxygen. The pKa scale tells the story — alkane C–H ≈ 50, acetone's α-H ≈ 19.3, and 1,3-dicarbonyls such as acetylacetone ≈ 9 (roughly \(10^{41}\) times more acidic than an alkane).
...
In a 1,3-dicarbonyl, the central CH₂ sits between two C=O groups; the enolate's negative charge delocalizes onto **either** oxygen, making the conjugate base exceptionally stable. The pKa drops from ~19 to ~9–13 — why malonate and acetoacetate esters are classic "activated methylene" compounds.
...
\[
K_{\text{eq}} = \frac{K_a(\text{HA})}{K_a(\text{HB})} = 10^{(pK_a(\text{HB}) - pK_a(\text{HA}))}
\]
...
\[
K_{\text{eq}} = 10^{(pK_a(\text{HB}) - pK_a(\text{HA}))} = 10^{(15.7 - 19.3)} = 10^{-3.6} \approx 2.5 \times 10^{-4}
\]
...
\[
K_{\text{eq}} = 10^{(pK_a(\text{amine}) - pK_a(\text{acetone}))} = 10^{(36 - 19.3)} = 10^{16.7} \approx 5 \times 10^{16}
\]
...
The same arithmetic explains why LDA is the standard base for ester enolates: ethyl acetate (pKa 25) with LDA gives \(K_{\text{eq}} = 10^{(36 - 25)} = 10^{11}\), essentially complete.
...
- \(K_{\text{eq}} = 10^{(pK_a(\text{HB}) - pK_a(\text{HA}))}\); show the formula, then substitute.
...
|---|---|---|
| pKa | −log₁₀(Ka); lower pKa = stronger acid. | The scale for comparing α-H acidity. |
| Enolate ion | The conjugate base of a carbonyl compound (negative charge on C and O by resonance). | The key nucleophile of carbonyl chemistry. |
| Resonance stabilization | Delocalization of charge over multiple atoms via π systems. | Explains why enolates are stable enough to form. |
| LDA (lithium diisopropylamide) | Very strong, bulky, non-nucleophilic base (conjugate acid pKa ≈ 36). | Standard base for complete, kinetic enolates. |
| Kinetic vs thermodynamic enolate | Fast deprotonation at the less hindered α-H vs the more substituted, more stable enolate. | Controls regiochemistry. |
| Activated methylene | A CH₂ flanked by two electron-withdrawing groups (malonate, acetoacetate). | pKa 9–13; deprotonated by simple alkoxides. |
...
3. Calculate \(K_{\text{eq}}\) for deprotonation of acetaldehyde (pKa 17) by ethoxide (EtOH pKa 16). Is this complete?
...
1. The enolate conjugate base is resonance-stabilized — the negative charge delocalizes onto the electronegative oxygen, stabilizing the deprotonated form and raising acidity. Ethane's carbanion has no such stabilization (pKa ≈ 50).
2. Increasing acidity (decreasing pKa): ethyl acetate (25) < acetone (19.3) < acetaldehyde (17) < acetylacetone (9).
3. \(K_{\text{eq}} = 10^{(pK_a(\text{HB}) - pK_a(\text{HA}))} = 10^{(16 - 17)} = 10^{-1} \approx 0.1\). Not complete: only ~10% of acetaldehyde is enolate at equilibrium; ethoxide is marginal for aldehydes.
4. LDA. NaOH (water pKa 15.7): \(K_{\text{eq}} \approx 10^{(15.7 - 19)} \approx 5 \times 10^{-4}\) (tiny). NaOEt (EtOH pKa 16): \(\approx 10^{-3}\) (tiny). LDA: \(10^{(36 - 19)} \approx 10^{17}\) (complete).
5. The kinetic enolate — deprotonation at the less hindered α carbon (more hydrogens, away from the methyl group). LDA is bulky and the reaction is fast and irreversible at −78 °C, so the product is controlled by how fast each α-H is removed, not by which enolate is more stable.

</details>

## Related Topics

- Previous: [Alpha Bromination of Carboxylic Acids](04-alpha-bromination-of-carboxylic-acids.md)
- Next: [Reactivity of Enolate Ions](06-reactivity-of-enolate-ions.md)
- Related: [Keto–Enol Tautomerism](01-keto-enol-tautomerism.md), [Reactivity of Enols: α-Substitution Reactions](02-reactivity-of-enols-substitution-reactions.md), [Alkylation of Enolate Ions](07-alkylation-of-enolate-ions.md)
...
- Content type: Original educational study guide based on the OpenStax outline structure. pKa values are standard textbook values; equilibrium constants are derived from \(K_{\text{eq}} = 10^{\Delta pK_a}\), not measured data.
- Lab safety note: LDA, NaH, and alkyllithiums are pyrophoric and moisture-sensitive; they require inert-atmosphere techniques and PPE per institutional rules. This guide states general principles only.
