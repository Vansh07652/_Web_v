...
> **Chapter:** [Chapter 6: Reproduction at the Cellular Level](../README.md)  
> **Topic:** 2  
> **Source structure:** OpenStax public textbook outline

## Overview

Every living thing — a bacterium, an oak tree, a person — exists because cells can make more cells. The **cell cycle** is the ordered sequence of events by which a eukaryotic cell duplicates its contents and divides to produce two daughter cells. It is the engine of growth and repair in multicellular bodies, and the basis of reproduction in single-celled eukaryotes.

The cycle is traditionally divided into two major parts. **Interphase** is the long preparation phase, during which the cell grows, performs its normal metabolic work, and copies its DNA. **M phase** (the mitotic phase) is the short division phase, during which the nucleus divides by **mitosis** and the cell splits in two by **cytokinesis**. Interphase itself is subdivided into G₁ (first gap), S (synthesis), and G₂ (second gap). The G stands for "gap," but those intervals are anything but empty: the cell is actively transcribing genes, building proteins, and stockpiling energy for division.
...
- **Exam staple:** The order of phases, what happens in S phase, and the names of the checkpoints appear on nearly every introductory biology exam.

## Core Concepts

### Interphase: preparation, not rest

- **G₁ phase:** The cell grows, synthesizes proteins and organelles, and performs its normal functions. Near the end of G₁, a cell in animal tissues faces the **G₁/S checkpoint** (sometimes called the restriction point): is the cell large enough, is the environment favorable, is the DNA undamaged? If the answers are no, the cell may exit the cycle into G₀ instead of committing to division.
- **S phase:** The defining event is DNA replication. Each chromosome is duplicated so that it now consists of two identical **sister chromatids** joined at a **centromere**. The centrosome, which will organize the spindle, also duplicates in S phase.
...
|---|---|---|
| Cell cycle | The ordered sequence of growth, DNA copying, and division that produces two daughter cells | It is the fundamental mechanism of growth, repair, and reproduction in eukaryotes |
| Interphase | The preparation phase (G₁, S, G₂) between divisions | Most of the cycle and most cellular work happen here, including DNA replication |
| S phase | The synthesis phase in which DNA is replicated | Doubling the genetic material is what makes equal division possible |
| Sister chromatids | Two identical copies of a chromosome joined at the centromere | Their separation in anaphase is how each daughter cell gets a full genome |
| Mitosis | Division of the nucleus into two genetically identical nuclei | It ensures chromosome number is preserved from parent to daughter cells |
| Cytokinesis | Division of the cytoplasm, producing two separate cells | Without it, mitosis would create one giant cell with two nuclei |
| Checkpoint | A molecular gate that halts the cycle until conditions are met | Defective checkpoints are a hallmark of cancer |
| Cyclin / CDK | Cyclin is a protein whose levels rise and fall, activating the cyclin-dependent kinase (CDK) that drives phase transitions | This pair is the engine that moves the cell through the cycle |

## Example: Healing a Paper Cut
...
|---|---|---|
| Interphase = "resting phase" | A metabolically quiet period | Interphase cells are busy growing, transcribing genes, and copying DNA; the name refers to being *between divisions*, not idle |
| The cell cycle = mitosis | The whole division process | Mitosis is only M phase's nuclear division; the cycle also includes interphase and cytokinesis |
| Sister chromatids = homologous chromosomes | Two copies of the *same* chromosome | Chromatids are identical duplicates joined at the centromere; homologs are the paired maternal/paternal chromosomes that carry different versions of genes |
| A checkpoint = a simple pause | An automatic stop | Checkpoints are active decisions made by cyclin-CDK machinery; the cell is held only until conditions pass inspection |
| G₀ = dead cell | A permanently stopped cell | G₀ cells are alive and functional; many (e.g., liver cells) can re-enter the cycle when signaled |
| Chromosome count doubles during S phase | DNA content doubles | The number of *chromosomes* stays the same; each one simply gains a sister chromatid |
...
1. G₁ → S → G₂ → M (mitosis + cytokinesis). During S phase, DNA is replicated so each chromosome becomes two sister chromatids.
2. The *number* of chromosomes is unchanged; each chromosome now consists of two sister chromatids. The chromosome number doubles only when the chromatids separate in anaphase and are counted as individual chromosomes.
3. G₁/S checkpoint: Is the cell large enough, the environment favorable, and the DNA intact? G₂/M: Was replication completed without damage? M (spindle) checkpoint: Are all chromatids properly attached to spindle fibers?
4. Animal cells pinch in with a cleavage furrow (contractile actin-myosin ring); plant cells build a cell plate that grows outward to divide the cell because their rigid wall prevents pinching.
5. G₀ is a nondividing but living state. Many cells (neurons, mature muscle) stay there permanently, while others (liver cells) can be recalled into the cycle. G₀ cells are functional, not dead.

</details>

## Related Topics

- Previous: [The Genome](01-the-genome.md)
- Next: [Cancer and the Cell Cycle](03-cancer-and-the-cell-cycle.md)
- Related: [Prokaryotic Cell Division](04-prokaryotic-cell-division.md) — how bacteria solve the same problem differently
- Related: [Reproduction at the Cellular Level chapter overview](../README.md)
